...
export default function GameweekBanner({ fixtures, lang }) {
  const [countdown, setCountdown] = useState('02:41:18')
  ...
  useEffect(() => {
    ...
    setCountdown(getCountdown(firstKickoff))
    const id = setInterval(() => setCountdown(getCountdown(firstKickoff)), 1000)
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 9, color: '#8FA99B', marginBottom: 1 }}>{lang.deadline ?? 'DEADLINE'}</div>
          <div style={{ fontFamily: 'Oswald', fontWeight: 700, fontSize: 15, color: '#F5C451' }}>
            {countdown}
          </div>
        </div>
      </div>
